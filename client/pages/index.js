...
export default function Home() {
  const styles = {
    PgCover: 'flex w-10/12 h-full bg-rose-30 items-center tablet:flex-col',
    Pg_lft: 'basis-4/6 bg-rose-60 h-5/6 p-5',
    Pg_rgt: 'basis-2/6 bg-rose-80 h-5/6 p-5 flex flex-col gap-10 w-full cstm_540:p-0',
    ...
    rgt_top: 'bg-rose-40 flex justify-end items-center tablet:justify-center',
    connectWalletBtn: 'text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-base px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2 tablet:m-5 tablet:text-sm',
    rgt_bottom: 'bg-rose-40 w-full',
    card: 'w-11/12 p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 mb-5 cstm_540:w-full',
    card_H: 'mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white',
    card_P: 'font-normal text-gray-700 dark:text-gray-400 text-sm leading-5 break-words',
    errorCover: '',
...
/* the statements of the king below makes infinite re-renders since 
...
thus being called almost the same time
  ...
  const getLotteryHistory = async (id) => {
    setLotteryHistory([])
    for (let i= parseInt(id); i> parseInt(id) - 3; i--) {
      const winnerAddress = await lcContract.methods.lotteryHistory(i).call()
/* creating winner obj with each iteration */
      const historyWinnersObj = {}
      historyWinnersObj.id= i
      historyWinnersObj.address = winnerAddress
/* appending winner obj to an array with each iteration */
      console.log(historyWinnersObj.id)
      console.log(historyWinnersObj.address)
      setLotteryHistory(lotteryHistory => [...lotteryHistory, historyWinnersObj]) 
    }
  }
  const getLotteryId = async() => {
    const lottery_id = await lcContract.methods.lotteryId().call()
    setLotteryId(lottery_id)
    await getLotteryHistory(lottery_id)
  }
  ...
  const payWinner = async () => {
    ...
    try {
      ...
      const winner_is = await lcContract.methods.lotteryHistory(lotteryId).call()
      setSuccessMsg(`the winner is ${winner_is}`)
      updateState()
    } catch (err) {
      setError(err.message)
    }
            ...
            <div>
              ...
              <div className={styles.lotteryInteract}>
                <div className={styles.section}>
                  <p className={styles.para}>Enter the lottery by sending 0.015 ether</p>
                ...
                <div className={styles.section}>
                  <p className={styles.para}><b>Admin only function:</b> pay the winner</p>
                  <button onClick={payWinner} className={styles.dropBtn}>
                    <span>Pay the Winner </span> <FaEthereum className='text-yellow-500 text-lg'/>
                    </button>
                </div>
              </div>
          ...
          <div className={styles.Pg_rgt}>
            ...
            <div className={styles.rgt_bottom}>
              <div className={styles.card}>
                ...
                {lotteryHistory.map((lotteryInstant, i) => {
                  if (lotteryInstant.id === lotteryHistory.length) return
                  return (
                    <div key={i}>
                      <p>Lottery #{lotteryInstant.id} winner:</p>
                      <a 
              ...
              <div className={styles.card}>
                <h5 className={styles.card_H}>Lottery Contract Address</h5>
                <p className={styles.card_P}>0x5be55a3f443A068d615fB59593d6E4E1e6DC7454</p>
              </div>
            </div>
          </div>
        </div>
